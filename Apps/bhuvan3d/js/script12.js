...
require(['jquery-ui'], function() {
	...
	var ps=new Cesium.Cartographic.fromDegrees(l,b,7000);
 ...
 var sm = viewer.entities.add({
		name : 'Salarjung Museum',
        position : Cesium.Cartesian3.fromDegrees(78.4803,17.3714),
         billboard : {
      image : 'http://bhuvan-staging.nrsc.gov.in/bhuvan2d/bhuvanpanorama/img/3dview.png',
      //image : 'http://bhuvan.nrsc.gov.in/map/bhuvannew/img/3dview.png',
      width : 20,
      height : 20
  }
  /*,
 ...
 var leftClickHandler = new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas); 
        ...
        leftClickHandler.setInputAction(function(action) { 
            ...
            if(pickedObject.id.name== 'Salarjung Museum'){ 
			window.open("http://bhuvan.nrsc.gov.in/map/bhuvannew/3dview/SM.html","Salarjung Museum","toolbar=no,location=no,status=no, menubar=no, scrollbars=yes,resizable=yes, top=200, left=200, width=800, height=600").focus(); 
			return; } 
...
$("#layers").html(" <input id='chkadminb' type='checkbox' style='float:left' ><label style='float:left'>Administrative Boundaries</label><br/><input id='chkbuilt' checked type='checkbox' style='float:left' ><label style='float:left'>Built-up Urban Models</label><label style='float:left'> (<a style='cursor:pointer' id='zbnglr' href='#'><u>Bangalore</u></a> &nbsp; <a style='cursor:pointer' id='zggn' href='#'><u>Gurgaon</u></a>&nbsp; <a style='cursor:pointer' id='zhyd' href='#'><u>Hyderabad</u></a>&nbsp; <a style='cursor:pointer' id='zkol' href='#'><u>Kolkatta</u></a> &nbsp; <a style='cursor:pointer' id='zmum' href='#'><u>Mumbai</u></a>)</label><br/><span style='color:red'>Footprints used for 3d view of built-up area are from OpenStreetMap Contributors.</span><br/><input id='chktrans' checked type='checkbox' style='float:left' ><label style='float:left'>Infrastructure (Road & Rail)</label><br/><input id='chkpanchayat' type='checkbox' style='float:left' ><label style='float:left'>Panchayat Boundaries</label><br/><input id='chksubbasin' type='checkbox' style='float:left' ><label style='float:left'>Sub Basin</label><br/><input id='chkbasin' type='checkbox' style='float:left' ><label style='float:left'>Basin</label><br/><input id='chkriver' type='checkbox' style='float:left' ><label style='float:left'>Rivers</label>");	
...
if(this.checked) loadlayer_gen("mmi:mmi_india","bhuvantransportnetwork","http://bhuvannuis.nrsc.gov.in/bhuvan/gwc/service/wms/"); 
else removelayer("bhuvantransportnetwork");
});	

$('#chkbuilt').click(function() { 
if(this.checked) loadlayer_gen("view_3d:view3d_city","bhuvanbuilt","http://bhuvannuis.nrsc.gov.in/bhuvan/gwc/service/wms/"); 

else removelayer("bhuvanbuilt");
});	

$('#chktrans').click();
$('#chkbuilt').click();
$('#chktrans').prop( "checked", true );
$('#chkbuilt').prop( "checked", true );
...
if(this.checked) loadlayer_gen("basemap:admin_group","bhuvanadminb","http://bhuvannuis.nrsc.gov.in/bhuvan/gwc/service/wms/"); 
 ...
 $('#zbnglr').click(function() {zoom_in(77.657, 12.838,7000);});
 $('#zggn').click(function() {zoom_in(77.069, 28.412,7000);});
 $('#zhyd').click(function() {zoom_in(78.3812,17.4498,7000);});
 $('#zkol').click(function() {zoom_in(88.429, 22.579,7000);});
 $('#zmum').click(function() {zoom_in(73.022, 19.017 ,7000);});
