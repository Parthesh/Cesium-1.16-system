...
define([
    ...
    var NavigationViewModel = function(canvas, cameraController) {
        ...
        var radius = this._ellipsoid.maximumRadius; //bhuvan 25-5
        this._zoomFactor = 1;
    ...
    function rotate3D(object, transform, constrainedAxis, restrictedAngle) {
        ...
        var rho = Cartesian3.magnitude(cameraController.position);
        ...
        if (defined(restrictedAngle)) {
            ...
            var direction = Cartesian3.clone(cameraController.directionWC, rotate3DRestrictedDirection);
           // var invTransform = transform.inverseTransformation();
			...
			//alert(invTransform+" inv transform");
    ...
    function zoom3D(object) {
        ...
        var camera = object._cameraController; //bhuvan 25-5
        var ellipsoid = object._ellipsoid;
    ...
    function tilt3D(object) {
        ...
        var height = ellipsoid.cartesianToCartographic(object._cameraController.position).height;
        ...
        if (defined(intersection)) {
           // center = ray.getPoint(intersection.start, tilt3DCenter); bhuvan 25-5
...
//alert("center outside");
	//	   alert(center);
       // var camera = cameraController._camera;
        var camera = cameraController;
        center = cameraController.worldToCameraCoordinates(new Cartesian4(center.x,center.y,center.z));
	//	alert("center  = " + center);
...
var res  = new Cartesian3();//bhuvan
        var angle = (minHeight * 0.25) / (Cartesian3.magnitude(Cartesian3.subtract(center, camera.position,res))); //bhuvan added res
    ...
    NavigationViewModel.prototype.update = function(mode) {
        ...
        if (mode === SceneMode.SCENE2D) {
            ...
            updateCV(this);
        } else { //bhuvan  if (mode === SceneMode.SCENE3D)
